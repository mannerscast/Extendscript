// ===============================================================
// Igniter Utilities - Comp Processor v3.0 (Safe Portable Version)
...
// - Verifies Output Module templates ("JPEG" and "ProRes SD") at load.
//
// ===============================================================
...
function igniterUtilities(thisObj) {
    ...
    function checkRenderTemplates() {
        var templates = app.project.renderQueue.itemTemplates; // doesn't list output modules, so use try/catch
        var result = { jpeg: false, prores: false };

        try {
            // create dummy comp for test
            var dummy = app.project.items.addComp("_templateTest", 100, 100, 1, 1, 30);
            var rq = app.project.renderQueue.items.add(dummy);
            ...
            // test ProRes SD template
            try {
                om.applyTemplate(CONFIG.RENDER.PRORES_SD_TEMPLATE);
                result.prores = true;
            } catch (e) {
                result.prores = false;
            }
            ...
            // fail silently, AE may disallow renderQueue ops before project saved
        }
    ...
    function processRenderComps() {
        ...
        try {
            ...
            if (!renderFolder) {
                infoBox.text = "Folder '" + CONFIG.FOLDER_NAMES.RENDER + "' not found.";
                return;
            }
    ...
    function prepRenderQueue() {
        ...
        try {
            var project = app.project;
            if (!project.file) {
                infoBox.text = "Save your project before running Render Queue Prep.";
                return;
            }
            var rq = project.renderQueue;
            if (rq.numItems === 0) {
                infoBox.text = "Render Queue is empty.";
                return;
            }
    ...
    function buildUI(thisObj) {
        ...
        // --- Check templates at load ---
        var templates = checkRenderTemplates();
        var msg = "Template Check: ";
        msg += "JPEG " + (templates.jpeg ? "OK" : "MISSING") + " | ";
        msg += "ProRes SD " + (templates.prores ? "OK" : "MISSING");
        infoBox.text = msg;

        // Button actions
        mainGroup.buttonGroup1.processButton.onClick = function() {
            if (app.project) processRenderComps(); else infoBox.text = "Open a project first.";
        };
        mainGroup.buttonGroup1.addThumbsButton.onClick = function() {
            if (app.project) addThumbsToRenderQueue(); else infoBox.text = "Open a project first.";
        };
        mainGroup.buttonGroup1.renderButton.onClick = function() {
            if (app.project) renderAndRenameThumbs(); else infoBox.text = "Open a project first.";
        };
        mainGroup.buttonGroup2.prepProjectButton.onClick = function() {
            if (app.project) prepProject(); else infoBox.text = "Open a project first.";
        };
        mainGroup.buttonGroup2.prepCompsButton.onClick = function() {
            if (app.project) prepComps(); else infoBox.text = "Open a project first.";
        };
        mainGroup.buttonGroup2.prepRenderQueueButton.onClick = function() {
            if (app.project) prepRenderQueue(); else infoBox.text = "Open a project first.";
        };
        mainGroup.buttonGroup2.createFoldersButton.onClick = function() {
            if (app.project) createFolders(); else infoBox.text = "Open a project first.";
        };
